export class GrokService {
     ...
     */
    async analyzeAndDecide(mention, thread) {
        if (this.simulationMode) {
     ...
     */
    buildAnalysisPrompt(mention, thread) {
     ...
     */
    parseGrokResponse(response, mentionPostId) {
        try {
     ...
     */
    simulateAnalysis(mention, thread) {
        ...
        const isQuestion = questionWords.some(word => mention.toLowerCase().includes(word));
        if (isQuestion) {
            const analysis = {
                action: {
                    type: 'reply',
