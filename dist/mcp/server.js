            ...
            try {
                switch (name) {
                    case 'x_fetch_mentions': {
                        const mentions = await this.xClient.fetchMentions(args.username);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'x_fetch_thread': {
                        const thread = await this.xClient.fetchThread(args.conversation_id);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'x_post_reply': {
                        const success = await this.xClient.postReply(args.in_reply_to_tweet_id, args.text);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'x_search_tweets': {
                        const results = await this.xClient.searchTweets(args.query);
                        return {
                            content: [
                                {
                                    type: 'text',
